...
const phrases = ["Full-Stack Developer", "Web Developer", "Software Developer"];
let phraseIndex = 0;
let charIndex = 0;
...
function type() {
    if (phraseIndex < phrases.length) {
        if (charIndex < phrases[phraseIndex].length) {
            document.getElementById("typed-text").textContent += phrases[phraseIndex].charAt(charIndex);
            charIndex++;
            ...
            setTimeout(() => {
                charIndex = 0;
                phraseIndex++;
                document.getElementById("typed-text").textContent = ''; // Clear text
                type(); // Start typing the next phrase
            }, 2000); // Pause before starting next phrase
        }
    } else {
        // Optionally restart after finishing all phrases
        setTimeout(() => {
            phraseIndex = 0;
            charIndex = 0;
            document.getElementById("typed-text").textContent = '';
            type();
        }, 100); // Pause before restarting
    }
}
